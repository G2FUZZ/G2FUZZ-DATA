...
# Generate a dummy FLV file with Script Data, Multi-bitrate Support, Data Rate Control, Video Tags with AVC (H.264), and AAC codec
...
with open('./tmp/sample_with_complex_file_structures.flv', 'wb') as f:
    # FLV header
    ...
    f.write(b'\x00\x00\x00\x00\x08')  # Tag Type: Video, DataSize: 8
    f.write(b'\x00\x00\x00\x00\x00\x00')  # Timestamp, StreamID
    f.write(b'\x17')  # Frame Type and CodecID (Keyframe, AVC)
    f.write(b'\x00\x00\x00\x00\x00\x00\x00\x00')  # Composition Time

    f.write(b'\x00\x00\x00\x18\x00\x00\x00\x00\x09')  # PreviousTagSize4

    f.write(b'\x00\x00\x00\x00\x08')  # Tag Type: Audio, DataSize: 8
    f.write(b'\x00\x00\x00\x00\x00\x00')  # Timestamp, StreamID
    f.write(b'\xaf')  # AAC Packet Type and CodecID

    f.write(flv_data)  # Existing FLV data

print("FLV file with Script Data, Multi-bitrate Support, Data Rate Control, Video Tags with AVC (H.264), and AAC codec generated successfully!")