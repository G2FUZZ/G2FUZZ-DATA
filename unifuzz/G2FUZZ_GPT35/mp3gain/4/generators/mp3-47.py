...
if audio_file is not None:
    ...
    # Custom text information frame
    custom_text_frame = eyed3.id3.frames.UserTextFrame(description='Additional Info', text='This mp3 file contains advanced features.')
    audio_file.tag.frame_set['TXXX'].append(custom_text_frame)

    # Add a comment frame
    comment_frame = eyed3.id3.frames.CommentFrame(language='eng', text='This is a custom comment associated with the audio.')
    audio_file.tag.frame_set['COMM'] = comment_frame

    # Add a popularimeter frame
    popularimeter_frame = eyed3.id3.frames.PopularimeterFrame(email='user@example.com', rating=192, counter=100)
    audio_file.tag.frame_set['POPM'] = popularimeter_frame
