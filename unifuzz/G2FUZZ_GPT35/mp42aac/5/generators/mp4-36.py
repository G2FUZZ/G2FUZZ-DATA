...
# Generate a more complex video with rotating rectangle, moving circle, changing colors, and text overlay
for i in range(100):
    ...
    cv2.circle(img, (width // 2, height // 2), radius, color, -1, lineType=cv2.LINE_AA, shift=0)  # Draw a filled circle
    cv2.addWeighted(img, 0.5, cv2.circle(img.copy(), (width // 2, height // 2), radius, color, -1), 0.5, 0, img)  # Blend two images
    text = f'Frame: {i}'
