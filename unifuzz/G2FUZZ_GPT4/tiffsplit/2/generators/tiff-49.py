...
spot_colors = [(0, 255, 255, 0)]  # Keeping the original spot color for simplicity
background_color = (0, 0, 0, 0)  # Pure black in CMYK

# Create an image with the original spot color
images = []
for color in spot_colors:
    spot_rectangle = (25, 25, 75, 75)  # Original spot position
    image = create_spot_color_image(width, height, spot_rectangle, color, background_color)
    images.append(image)

# Save the image as a multi-frame TIFF, although in this case it's just a single image for compatibility
