    ...
    def _loop(self) -> None:
        ...
        if cores_left > 0:
            afl_cmd = [
                '/usr/bin/sudo',
                Path('~/fuzztruction/target/debug/fuzztruction').expanduser().resolve().as_posix(),
                self._target.config.as_posix().replace(".yml", "-g2fuzz.yml"),
                '--suffix', self.suffix(),
                'aflpp',
                '-t', f'{self._timeout_s}s',
                '-j', str(cores_left)
...
class AflPlusPlusJobRaw(FuzzingJob):
...
class FuzztructionJob(AflPlusPlusJobRaw):
...
class WeizzJob(AflPlusPlusJobRaw):
    ...
    #     .value_of("weizz-jobs")
    #     .map(|e| e.parse().unwrap())
class SymccJob(AflPlusPlusJobRaw):
